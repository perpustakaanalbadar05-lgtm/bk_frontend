    ...
    >
      ...
      <div className={`flex items-center gap-3 p-5 border-b border-white/10 ${collapsed ? 'lg:justify-center lg:px-2' : ''}`}>
        <div className="relative flex-shrink-0">
          <div className="w-10 h-10 rounded-xl bg-gradient-to-br from-primary-500 to-accent-600 flex items-center justify-center shadow-glow-sm">
        ...
        {!collapsed && (
          <div className="flex-1 min-w-0">
            ...
            <p className="text-dark-400 text-[10px] truncate leading-tight">CV. Alifba Media</p>
          </div>
        )}
        {/* Close on mobile */}
        <button
          onClick={onClose}
          className="lg:hidden ml-auto p-1.5 rounded-lg text-dark-400 hover:text-white hover:bg-white/10 transition-colors"
        >
        ...
        <button
          onClick={onCollapse}
          className="hidden lg:flex p-1.5 rounded-lg text-dark-400 hover:text-white hover:bg-white/10 transition-colors ml-auto"
        >
      ...
      <nav className="flex-1 p-3 space-y-1 overflow-y-auto">
        {!collapsed && (
          <p className="text-dark-500 text-[10px] font-semibold uppercase tracking-wider px-3 pb-2 pt-1">
            Menu Utama
          </p>
        )}
      ...
      {/* User profile */}
      <div className={`p-3 border-t border-white/10 ${collapsed ? 'lg:flex lg:justify-center' : ''}`}>
        {!collapsed ? (
          <div className="flex items-center gap-3 p-3 rounded-xl glass">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-dark-400 text-xs truncate">{user?.email || 'guru@sekolah.id'}</p>
            </div>
            <button
          ...
          <button
            onClick={handleLogout}
            className="p-2.5 rounded-xl text-dark-400 hover:text-red-400 hover:bg-red-500/10 transition-colors"
            title="Keluar"
          >
